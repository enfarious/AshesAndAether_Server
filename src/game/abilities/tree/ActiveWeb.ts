...
export const ACTIVE_WEB: AbilityNode[] = [
...

  {
    ...
    name: 'Provoke',
    description: 'Force a single enemy to attack you for a short time.',
    cost: TIER_COST[1],
    adjacentTo: [a('support', 't1'), a('phys', 't1'), a('tank', 't2a'), a('tank', 't2b')],
    activeEffect: {
      description: 'Taunt single target — forces attention for 8 s.',
...

  {
    ...
    name: 'Power Strike',
    description: 'A heavy melee blow that hits harder than a basic attack.',
    cost: TIER_COST[1],
    adjacentTo: [a('tank', 't1'), a('control', 't1'), a('phys', 't2a'), a('phys', 't2b')],
    activeEffect: {
      description: 'Heavy physical hit — 150% base attack rating.',
      targetType: 'enemy', range: 3, damageType: 'physical',
      staminaCost: 20, cooldown: 8,
    },
  },
...

  {
    ...
    name: 'Ensnare',
    description: 'Entangle a target in place, slowing their movement significantly.',
    cost: TIER_COST[1],
    adjacentTo: [a('phys', 't1'), a('magic', 't1'), a('control', 't2a'), a('control', 't2b')],
    activeEffect: {
      description: 'Root + heavy slow on single target for 5 s.',
      targetType: 'enemy', range: 15,
      manaCost: 20, cooldown: 16,
      statusEffects: ['root', 'slow_heavy'],
    },
  },
...

  {
    ...
    name: 'Arcane Bolt',
    description: 'A focused burst of arcane energy hurled at a single target.',
    cost: TIER_COST[1],
    adjacentTo: [a('control', 't1'), a('healer', 't1'), a('magic', 't2a'), a('magic', 't2b')],
    activeEffect: {
      description: 'Single-target magic damage — 120% magic attack.',
      targetType: 'enemy', range: 25, damageType: 'magic',
...

  {
    ...
    name: 'Mend',
    description: 'Restore a portion of a single ally\'s health.',
    cost: TIER_COST[1],
    adjacentTo: [a('magic', 't1'), a('support', 't1'), a('healer', 't2a'), a('healer', 't2b')],
    activeEffect: {
      description: 'Heal single target — 120% wisdom.',
...

  {
    ...
    name: 'Encourage',
    description: 'Bolster an ally\'s fighting spirit, raising their attack power.',
    cost: TIER_COST[1],
    adjacentTo: [a('healer', 't1'), a('tank', 't1'), a('support', 't2a'), a('support', 't2b')],
    activeEffect: {
      description: 'Apply atk-up buff to single ally for 15 s.',
      targetType: 'ally', range: 20,
      manaCost: 18, cooldown: 20,
      statusEffects: ['atk_up'],
    },
  },
