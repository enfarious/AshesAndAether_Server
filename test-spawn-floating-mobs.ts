...
async function testGravityWithFloatingMobs() {
  ...
  // Get terrain metadata for ground level calculation
  const metadata = elevationService.getMetadata();
  const groundLevel = metadata.center ? elevationService.getElevationMeters(metadata.center.lat, metadata.center.lon) || 265 : 265;
  ...
  for (const mobDef of testMobs) {
    const mob = await prisma.mob.create({
      data: {
        ...
        tag: `test.floating_rat.${Date.now()}.${Math.random()}`, // Unique tag per mob
        level: 1,
        stats: {
          strength: 6,
          vitality: 6,
